...
const getDisabledStyles = ({ $isLoading, $textVisible }: TransientButtonProps) => {
  if ($isLoading === true) {
    return `
      &:disabled,
      &.button--disabled {
      ...
      &.button--loading {
        position: relative;
        color: ${!$textVisible ? 'transparent !important' : 'inheret'};
        .loader {
          position: ${!$textVisible ? 'absolute' : 'relative'};
        }
      }
    `;
  }
